...
class Reglages(tk.Toplevel):
    ...
    def lancement(self, reglages):
        ...
        self.ajout_separation(
            position=2, boss=self, color="green", is_separation_principale=True
        )
    ...
    def lancement_tous_les_reglages(self, reglages):
        ...
        for regl in reglages:
            ...
            self.tous_les_reglages[reglage.name].init_entitees_de_base(
                self, self.big_boss, self.fenetre
            )
    ...
    def resize_canvas(self):
        self.canvas.configure(
            width=self.content.winfo_width(), height=self.content.winfo_height()
        )
    ...
    def ajout_separation(
        self, position, boss=None, color="grey", is_separation_principale=False
    ):
        if boss is None:
            boss = self.content
    ...
    def init_header(self):
        ...
        btn = tk.Button(
            self.header,
            text="Tout",
            command=self.afficher_tous_params,
            font=("Helvetica", 14),
            bg="green",
            fg="white",
        )
        ...
        self.combobox_header = ttk.Combobox(
            self.header,
            values=noms_reglages,
            state="readonly",
            justify="center",
            width=15,
            height=15,
        ...
        self.combobox_header.bind(
            "<<ComboboxSelected>>", self.affiche_type_reglage_particulier
        )
        ...
        btn = tk.Button(
            self.header,
            text="Appliquer",
            command=self.appliquer_modifications,
            font=("Helvetica", 14),
            bg="blue",
            fg="white",
        )
        ...
        self.var_alerte_mauvaise_entree.set(
            int(self.big_boss.parametres["lab alea alerte mauvaise entree"])
        )
        checkbtn_alerte_mauvaise_entree = tk.Checkbutton(
            self,
            variable=self.var_alerte_mauvaise_entree,
            compound=tk.LEFT,
            text="Alerte pour mauvaise entrée",
            border=10,
            font=("Helvetica", 13),
        )
    ...
    def appliquer_modifications(self):
        ...
        self.big_boss.parametres["lab alea alerte mauvaise entree"] = (
            self.var_alerte_mauvaise_entree.get()
        )
