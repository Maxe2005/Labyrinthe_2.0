...
class Boutons(tk.Frame):
    ...
    def def_bouton(
        ...
        if type_combobox:
            btn = Bcombobox(
                boss, effet, position, visibilite, nom_affiche, values=type_combobox
            )
    ...
    def redimentionner(self, text_size=None):
        ...
        for bout in self.items:
            ...
            if self.items[bout].commentaire:
                self.items[bout].commentaire.commentaire_label.config(
                    font=("Verdana", text_size)
                )
