...
class Infos(tk.Toplevel):
    def __init__(
        ...
        self.text = tk.Text(
            self,
            wrap=tk.WORD,
            width=int((pourcentage_largeur / 100) * taille_ligne_max),
            height=texte.count("\n") + 6,
            padx=30,
            pady=30,
            font=(police, taille_police),
            bg=color,
        )
        ...
        self.text.tag_config(
            "titre", font=police + " " + str(taille_police + 2), justify=tk.CENTER
        )
...
class Infos_generales(tk.Toplevel):
    ...
    def init_titre_et_texte(
        ...
        text.tag_config(
            "titre", foreground="red", font=("Helvetica", 20), justify="center"
        )
