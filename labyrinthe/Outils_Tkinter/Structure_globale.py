            ...
            for param in self.parametres:
                if type(self.parametres[param]) is list:
                    f.write(
                        ...
                        + param
                        + ","
                        + ",".join(self.parametres[param])
                        + "\n"
                    )
                else:
                    f.write(
                        "parcoureur," + param + "," + str(self.parametres[param]) + "\n"
                    )
...
class Fenetre(tk.Tk):
    ...
    def _set_window_icon(self, image_path):
        """Définit l'icône de la fenêtre avec fallback pour Linux"""
        import os

        # Construire le chemin vers le fichier ICO
        path_split = image_path.split("/")
        path_ico = (
            "/".join(path_split[:-1])
            + "/Fichiers_ico/"
            + path_split[-1].split(".")[-2]
            + ".ico"
        )

        # Première tentative : utiliser le fichier ICO (fonctionne mieux sur Windows)
        if os.path.exists(path_ico):
            try:
                self.iconbitmap(path_ico)
                return
            except tk.TclError as e:
                print(f"Impossible d'utiliser le fichier ICO: {e}")
        return
