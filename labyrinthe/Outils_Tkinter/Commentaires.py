...
class Commentaire(tk.Toplevel):
    def __init__(
        ...
        widget,
        texte: str,
        aligne_in: Literal["center", "left", "right", "top", "bottom"] = "center",
        position_out: list = ["L", "B", "R", "T"],
    ) -> None:
        ...
        #     just = tk.BOTTOM
        self.commentaire_label = tk.Label(
            self, text=texte, justify=just, bg="grey", fg="white"
        )
    ...
    def afficher_commentaire(self):
        if self.affichage_possible:
            ...
            i = 1
            self.conditions(x, y)
            while (self.at_least_one_corner_in or self.passing_in_front) and i < len(
                self.position_out
            ):
    ...
    def conditions(self, x, y):
        ...
        x2_in = (
            self.pos_x < x + self.widget.winfo_width() < self.pos_x + self.winfo_width()
        )
        y1_in = self.pos_y < y < self.pos_y + self.winfo_height()
        y2_in = (
            self.pos_y
            < y + self.widget.winfo_height()
            < self.pos_y + self.winfo_height()
        )
        ...
        self.passing_in_front = (
            passing_in_front_x or passing_in_front_y or hide_completely
        )

    def def_pos_left(self, x, y):
        self.pos_x = x - self.winfo_width() - self.marge_bouton
        self.pos_y = round(
            y + (self.widget.winfo_height() / 2) - self.winfo_height() / 2
        )
    ...
    def def_pos_bottom(self, x, y):
        self.pos_x = round(
            x + (self.widget.winfo_width() / 2) - (self.winfo_width() / 2)
        )
    ...
    def verif_not_out_window(self):
        ...
        if self.pos_x - marge < self.fenetre.winfo_rootx():
            ...
            self.pos_x + self.winfo_width() + marge
            > self.fenetre.winfo_rootx() + self.fenetre.winfo_width()
        ):
            self.pos_x = (
                self.fenetre.winfo_rootx()
                + self.fenetre.winfo_width()
                - self.winfo_width()
                - marge
            )
        if self.pos_y < self.fenetre.winfo_rooty() - marge:
            ...
            self.pos_y + self.winfo_height() + marge
            > self.fenetre.winfo_rooty() + self.fenetre.winfo_height()
        ):
            self.pos_y = (
                self.fenetre.winfo_rooty()
                + self.fenetre.winfo_height()
                - self.winfo_height()
                - marge
            )
